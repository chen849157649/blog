...
1、脏读。就是事务读取了其他事务未提交的数据，[上一篇文章《一条SQL语句提交后，db2都做了什么？》](/views/articles/2018/db2_memory.html)中提到，数据库中增删改都是在缓冲区中进行的，查询操作也是优先去缓冲区中查找，如果读取了未提交时数据可能是不正确的，因为未提交的数据随时有可能回退，一旦回退，读取的数据肯定是无效的。
